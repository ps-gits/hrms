$(function () {
  ...
  var dtTable = $('.datatables-deviceStatus');
  ...
  if (dtTable.length) {
    ...
    var dtDeviceStatus = dtTable.DataTable({
      processing: true,
      serverSide: true,
      ajax: {
        url: baseUrl + 'device/indexAjax',
        data: function (d) {
      ...
      columnDefs: [
        ...
        {
          //TODO: On click open full screen using a plugin
          //Image
          targets: 6,
          className: 'text-start',
          render: function (data, type, full, meta) {
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            return (
              ...
              `<button class="btn btn-sm btn-icon me-1 device-status-details" data-id="${full['id']}" data-bs-toggle="offcanvas" data-bs-target="#offcanvasDeviceStatusDetails"><i class="bx bx-show"></i></button>` +
  ...
  $(document).on('click', '.device-status-details', function () {
    var id = $(this).data('id');
    $.get(`${baseUrl}device/getByIdAjax/${id}`, function (response) {
  ...
  $(document).on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        $.ajax({
          type: 'DELETE',
          url: `${baseUrl}device/deleteAjax/${id}`,
          success: function () {
            // success sweetalert
            Swal.fire({
              icon: 'success',
