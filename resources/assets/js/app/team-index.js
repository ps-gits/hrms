...
$(function () {
...

        {
          ...
          render: function (data, type, full, meta) {
            return (
              ...
              `<button class="btn btn-sm btn-icon edit-record" data-id="${full['id']}" data-bs-toggle="offcanvas" data-bs-target="#offcanvasAddOrUpdateTeam"><i class="bx bx-pencil"></i></button>` +
  ...
  $(document).on('click', '.add-new', function () {
    $('#id').val('');
    $('#notes').val('');
    $('#offcanvasTeamLabel').html(('Add Team'));
    fv.resetForm(true);
    setCode();
  ...
  function setCode(){
   $.get(`${baseUrl}teams\/getCodeAjax`, function (data) {
      $('#code').val(data);
    });
  }
   ...
   $('#isChatEnabledToggle').on('change', function() {
      ...
      if (this.checked) {
        $('#isChatEnabledToggle').prop('checked', true);
         $('#isChatEnabled').val(1);
      }
   });
  ...
  const fv = FormValidation.formValidation(addTeamForm, {
    fields: {
      name: {
        validators: {
          notEmpty: {
      ...
      code: {
        validators: {
          ...
          remote: {
            ...
            data: function () {
              return {
                id: addTeamForm.querySelector('[name="id"]').value
              };
            }
          }
        }
  ...
  $(document).on('click', '.delete-record', function () {
   ...
   $(document).on('change', '.status-toggle', function() {
    ...
    $.ajax({
      url: `${baseUrl}teams/changeStatus/${id}`,
      type: 'POST',
      data: {
        status: status,
